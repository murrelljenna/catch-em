...
pub fn send_player_input(
    socket: Res<Socket>, keys: Res<Input<KeyCode>>, mut transport: ResMut<Transport>
) {
    transport.send(socket.0.peer_addr().expect("Socket address could not be found"), &serialize(NetworkInput {
